...
const PlanetListPage = (props) => {
    ...
    const voltarPagina = () => {
        if (PlanetData.previous !== null) {
            setPageURL(PlanetData.previous.replace(BaseURL+"/", ""))
            setNumPagina(PlanetData.previous.replace(BaseURL+"/?page=", ""))
        }
    }

    const proximaPagina = () => {
        if (PlanetData.next !== null) {
            setPageURL(PlanetData.next.replace(BaseURL+"/", ""))
            setNumPagina(PlanetData.next.replace(BaseURL+"/?page=", ""))
        }
    }
